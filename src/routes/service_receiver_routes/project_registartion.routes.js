...
const routes = (router) => {
            ...
            Project_Registration.findOne({}, { regNo: 1 })
                ...
                .then(maxres => {
                    ...
                    obj.save().then(result => {
                        res.json({ statuscode: 200, message: `Data saved Successfully`,Result: result });
                    }).catch(er => {
    ...
    router.delete('/delete_projects/:id', (req, res) => {
        try {
            Project_Registration.findByIdAndDelete({ _id: req.params.id }).then(result => {
                res.json({ statuscode: 200, message: `Data Updated Successfully` });
            }).catch(err => {
                res.json({ statuscode: 402, message: `Error is : ${err}` });
            })
